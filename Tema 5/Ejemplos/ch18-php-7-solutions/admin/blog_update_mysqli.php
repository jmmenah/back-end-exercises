...
if (($done || $trans_error ) || (!$_POST && !isset($_GET['article_id']))) {
    $url = 'http://localhost/ch18-php-7-solutions/admin/blog_list_mysqli.php';
    if ($done) {
        $url .= '?updated=true';
    } elseif ($trans_error) {
...
if (isset($catError)) {
    echo "<p class='warning'>Error: $catError</p>";
}
if (isset($transactionError)) {
    echo "<p class='warning'>Error: $transactionError</p>";
}
if($article_id == 0) { ?>
...
<form method="post" action="blog_update_mysqli.php">
    ...
    <p>
        ...
        <textarea name="article" id="article"><?php
            if (isset($article)) {
                echo safe($article);
            } else {
